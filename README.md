# My Portfolio

<img width="800" height="800" alt="Bawo LinkedIn Profile Picture" src="https://github.com/user-attachments/assets/567ecd8a-6e31-4a45-83c0-37175038d0b6" style="float: right; width: 200px; height: 200px; object-fit: cover; border-radius: 50%; margin-left: 20px; margin-bottom: 20px;"/>

# Bawo Ogharandukun | Data & BI Analyst
### Turning raw business data into decisions that drive growth, retention, and revenue.

I’m a Data & BI Analyst with a background in administrative operations and business support, which gives me a practical understanding of how companies actually use data to make decisions.  

I specialize in transforming messy datasets into clear dashboards, actionable insights, and business-focused reports using SQL, Excel, Power BI, and Python.

📍 Lagos, Nigeria  
📧 [Email Me](mailto:ogharandukunbawo@gmail.com)  
💼 [LinkedIn](https://linkedin.com/in/bawo-ogharandukun)  
📄 [View Resume](https://drive.google.com/file/d/1H6rdHuUq1b8niJgwNNhJtx8ymFlgQjhP/view?usp=sharing)

---

# 📊 Featured Projects

## 1. Telecom Customer Churn Analysis
### Reducing Customer Churn Through Data-Driven Insights

<img width="994" height="597" alt="image" src="https://github.com/user-attachments/assets/0a35ad16-64de-4641-8fcf-6537d31fc07a" />


### 🎯 Business Problem

The telecom company was experiencing customer loss without a clear understanding of:
- Why customers were leaving
- Which customer groups were most at risk
- What services or payment patterns influenced churn
- How to proactively identify future churners

  ## 🔍 What I Did
- Built a complete end-to-end churn analysis solution using **MicrosoftSQL, Power BI, and Python**
- Cleaned and transformed telecom customer data using **SQL** and **Power Query**
- Created SQL views and ETL processes for structured reporting
- Built an interactive **Power BI dashboard** to analyze:
  - Churn trends
  - Customer demographics
  - Contract types
  - Payment methods
  - Monthly charges
  - Customer tenure etc
- Developed a **Random Forest Machine Learning model** in Python to predict future churners
- Created a churn prediction dashboard to identify high-risk customers
- Identified key churn drivers through exploratory analysis and KPI tracking

### 📈 Key Insights
- Customers on month-to-month contracts had the highest churn rate
- Higher monthly charges strongly correlated with customer exits
- Long-term contract customers showed significantly better retention

### 💡 Business Impact
The dashboard provided management with a clearer retention strategy by identifying high-risk customer segments and opportunities to improve customer loyalty.

### 🛠 Tools Used
`Power BI` • `Python` • `Pandas` • `Excel` • `Power Query`

🔗 [View Dashboard](https://drive.google.com/drive/folders/1rE_emq1qurfCdqdWJ2EcnUPQWDFDmHEZ?usp=drive_link)  
💻 [View Code](https://github.com/bawo7/Telecom-Customer-Churn-Analysis-Prediction)


---

# 🧠 Skills & Tools

### Data Analysis
- SQL
- Python
- Excel
- Power Query
- Data Cleaning
- Exploratory Data Analysis

### Business Intelligence
- Power BI
- Dashboard Design
- Data Storytelling
- KPI Reporting
- DAX

### Professional Skills
- Business Communication
- Stakeholder Reporting
- Problem Solving
- Administrative Operations
- Project Coordination

---

# 📌 What Makes Me Different

My background in administrative operations helps me bridge the gap between data and business needs. I don’t just build dashboards, I build reports and insights that decision-makers can actually understand and use.

I focus on:
- Clear storytelling
- Business-focused insights
- Clean and professional dashboard design
- Actionable recommendations instead of just numbers

---

# 📬 Let's Connect

I’m passionate about using data to solve real business problems and create impactful dashboards that tell clear stories.

If you're hiring for internship, entry-level, or remote data opportunities, I’d love to connect.

📧 [Send Me an Email](mailto:ogharandukunbawo@gmail.com)  
💼 [LinkedIn](https://linkedin.com/in/bawo-ogharandukun)
